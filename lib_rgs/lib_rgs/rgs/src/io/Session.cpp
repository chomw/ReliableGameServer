...
Session::read(const rgs::protocol::RowData& rowData, unsigned int& readBytes)
{
	...
	while (true)
	{
		...
		{
			RGS_LOG(severity_level::error) << "Session-" << id_ << " Session::read() Error : " << rgs::Error::ERROR_DISCONNECTED_SESSION;
		...
		if (error)
		{
			RGS_LOG(severity_level::error) << "Session-" << id_ << " Session::read() Error : " << error;
			return error;
		}
...
Session::dispatch()
{
	...
	while (packets_.empty() == false)
	{
		...
		if (packet == nullptr)
		{
			RGS_LOG(severity_level::error) << "Session-" << id_ << " Session::dispatch() Error : " << rgs::Error::ERROR_NULL_DATA;
			continue;
		}
...
Session::complete(rgs::io::IoEvent ioEvent)
{
	if (ioEvent == rgs::io::IoEvent::IO_EVENT_CONNECTED)
	{
		...
		RGS_LOG(severity_level::info) << "Session-" << id_ << " is Connected (host : " <<
	...
	{
		RGS_LOG(severity_level::info) << "Session-" << id_ << " is Disconnected (host : " <<
			ipAddress_.host << " / port : " <<
			ipAddress_.port << ")";
