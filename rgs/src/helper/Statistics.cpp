...
bool SetPrivilege(HANDLE hToken, LPCWSTR pszPrivilege, BOOL bEnablePrivilege)
{
	...
	{
		RGS_LOG(severity_level::error) << "LookupPrivilegeValue error" << GetLastError();
		return false;
	}
	...
	{
		RGS_LOG(severity_level::error) << "AdjustTokenPrivileges error" << GetLastError();
		return false;
	}
...
bool Statistics::memoryUsage(int& workingSet, int& privateUsage)
{
	...
	if (!OpenThreadToken(GetCurrentThread(), TOKEN_ADJUST_PRIVILEGES | TOKEN_QUERY, FALSE, &hToken))
	{
		if (GetLastError() == ERROR_NO_TOKEN)
		{
			...
			{
				RGS_LOG(severity_level::error) << "ImpersonateSelf error(" << GetLastError() << ") #1";
				return false;
			}

			if (!OpenThreadToken(GetCurrentThread(), TOKEN_ADJUST_PRIVILEGES | TOKEN_QUERY, FALSE, &hToken))
			{
				RGS_LOG(severity_level::error) << "OpenThreadToken error(" << GetLastError() << ") #2";
				return false;
			}
		}
		else
		{
			RGS_LOG(severity_level::error) << "OpenThreadToken error(" << GetLastError() << ") #1";
			return false;
		}
	}
	...
	{
		RGS_LOG(severity_level::error) << "CreateToolhelp32Snapshot error";
		return false;
	}
	...
	{
		RGS_LOG(severity_level::error) << "Process32First error";
	}
	else
	{
		do
		{
			...
			if (hProcess)
			{
				...
				if (appName_ == processName)
				{
					...
					{
						RGS_LOG(severity_level::core) << L"GetProcessMemoryInfo error(" << GetLastError() << ")";
					}
				}
